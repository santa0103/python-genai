...
class InteractionsResource(SyncAPIResource):
  ...
  def create(
    ...
    Args:
      ...
      background: Input only. Whether to run the model interaction in the
        background.
      environment: The environment configuration for the interaction. Can be an
        object specifying remote environment sources or a string referencing an
  ...
  def get(
    ...
    Args:
      ...
      last_event_id: Optional. If set, resumes the interaction stream from the
        next chunk after the event marked by the event id. Can only be used if
        `stream` is true.
